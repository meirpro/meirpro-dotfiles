...
# the same lint logic on every edit. The project hook lives in its repo
# (committed) so teammates without these global hooks still get
# lint-on-edit; the global one stays as the default for projects that
